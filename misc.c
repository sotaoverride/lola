...
get_time (void)
{
  ...
  return ((uint64_t) 1000000 << 32u) | 2000000;
}
...
BuildSentence (TGPS *GPS, char *TxLine, const char *PayloadID)
{
  ...
  sprintf (
      ...
      PayloadID, SentenceCounter, GPS->Hours, GPS->Minutes, GPS->Seconds,
      GPS->Latitude, GPS->Longitude, GPS->Altitude, GPS->Satellites,
      GPS->BatteryVoltage, GPS->InternalTemperature, GPS->ExternalTemperature,
      GPS->Pressure, GPS->Humidity, GPS->CDA, GPS->PredictedLatitude,
      GPS->PredictedLongitude, GPS->PredictedLandingSpeed,
      GPS->TimeTillLanding);
